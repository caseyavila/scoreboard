...
int main(int argc, char *argv[]) {
    ...
    std::vector<std::vector<pixel>> all_frames = parse_all_frames("../frames/ball-bounce/");
    std::vector<pixel> spd_frame = parse_frame("../frames/spd-frame");
    ...
    while (!interrupt_received && running) {
        if (update) {
            ...
            for (pixel px : all_frames[frame_index]) {
                canvas->SetPixel(px.x, px.y, px.red, px.green, px.blue);
            }
            frame_index = (frame_index + 1) % all_frames.size();
